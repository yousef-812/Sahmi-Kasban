...
def test_shared_computation_cache_charges_each_account_once(
    client: TestClient,
    fake_email_service,
    db_session: Session,
) -> None:
    ...
    assert repeated.json()["charged_points"] == 0
    ...
    wallets = db_session.scalars(select(WalletAccount).order_by(WalletAccount.id)).all()
    assert len(debits) == 2
